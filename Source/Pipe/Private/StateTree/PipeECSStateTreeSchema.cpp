...
namespace p
{
	p::IdContext& GetIdContext(FStateTreeExecutionContext& Context)
	{
	...
	p::Id GetOwnerId(FStateTreeExecutionContext& Context)
	{
		const FStateTreeDataView View =
...
bool UPipeECSStateTreeSchema::IsStructAllowed(const UScriptStruct* InScriptStruct) const
{
	return InScriptStruct->IsChildOf(FStateTreeConditionCommonBase::StaticStruct()) ||
		   ...
		   InScriptStruct->IsChildOf(FStateTreePropertyFunctionPipeECSBase::StaticStruct());
}
